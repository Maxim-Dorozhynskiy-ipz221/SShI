...
DATA_FILE = Path(r"D:\СШІ\Lab5\traffic_data.txt")
...
def read_any_csv(path: Path) -> pd.DataFrame:
    with open(path, "r", encoding="utf-8") as f:
        ...
        df = pd.read_csv(f, sep=dialect.delimiter, encoding="utf-8", on_bad_lines="skip")
...
def find_target_column(df: pd.DataFrame) -> str:
    ...
    raise ValueError(f"Не знайдено цільову колонку. Колонки: {cols}")
...
def load_and_encode(path: Path):
    ...
    X, y = X[mask], y[mask]
    print(f"Цільова колонка: '{target_col}'")
...
def main():
    ...
    X, y, encoders, cats, nums = load_and_encode(DATA_FILE)
